...
{
    add_cors_headers(req);
    const char *json_resp = "{\"version\":\"1.0.10\",\"build_number\":10,\"apk_url\":\"http://192.168.11.51:8080/downloads/app-debug.apk\",\"changelog\":\"🌧️ Bản nâng cấp v1.0.10: Hiệu ứng Thời Tiết 3D & Giọt Nước bám kính Liquid Glass tương tác sinh động!\"}";
    httpd_resp_set_type(req, "application/json");
    return httpd_resp_send(req, json_resp, HTTPD_RESP_USE_STRLEN);
}
